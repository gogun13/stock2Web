...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#btnSearch').click(function(){
			...
			try{
				...
				gp_postAjaxRequest("#frm", "#invoiceCredit-list",function(){
					$("#productCodeDis").val('');
					$("#productCodeDis").focus();
				});
		...
		$('#btnPrint').click(function(){ 
			...
			try{
				...
				var lv_action = '<s:url action="invoiceCreditAction.action" />';
	...
	function lp_addRowtable(){
		try{
			...
			gp_postAjaxRequest("#frm", "#invoiceCredit-list");
		}catch(e){
	...
	function lp_getMultiProductDetailFromList(){
		...
		try{
			...
			gp_postAjaxRequest("#frm", "#invoiceCredit-list",function(data){
				try{
					$("input[name=rowIndex]").each(function() {
	...
	function lp_calInvoiceVat(){
		...
		try{
			...
			$( 'input[name="invoiceCreditMasterBean\\.invoiceType"]:checked' ).each(function( index ) {
				if($(this).val()=="V"){
	...
	function lp_setStatus(){
		try{
			$("#invoiceCreditMasterBean\\.invoiceStatus").val($("#invoiceStatus").val());
		}catch(e){
...
<s:form name="frm" id="frm" action="invoiceCreditAction" onsubmit="return false;">
			...
			<div class="panel-body">
				  ...
				  <div class="panel-body paddingForm">
					...
					<div class="col-sm-12">
						...
						<div class="col-sm-3 text-left">
							<s:textfield id="invoiceCode" name="invoiceCreditMasterBean.invoiceCode" cssClass="form-control input-disabled" readonly="true" placeholder="เลขที่ใบเสร็จ"  />
						</div>
						<div class="col-sm-2 text-right">
						...
						<div class="col-sm-3 text-left">
							<s:if test='"NEW".equals(pageMode) || "EDIT_PAGE".equals(pageMode)'>
								<s:textfield id="invoiceDate" name="invoiceCreditMasterBean.invoiceDate" onchange="lp_getMultiProductDetailFromList();" cssClass="form-control input-disabled dateFormat" cssStyle="width:85%;display:-webkit-inline-box;" readonly="true" placeholder="วันที่ขาย" />
							</s:if>
							<s:else>
								<s:textfield id="invoiceDate" name="invoiceCreditMasterBean.invoiceDate" cssClass="form-control input-disabled" readonly="true" placeholder="วันที่ขาย" />
							</s:else>
						</div>
					</div>
					<%--End Row 1 --%>
				...
				<div class="panel panel-primary" style="border: none;box-shadow: none;">
				  ...
				  <div class="panel-body paddingForm">
					...
					<div class="col-sm-12">
						<div class="col-sm-2 text-right">
							<label for="invoiceCreditMasterBean.invoiceTypeV" class="control-label">
								ประเภทราคา:
							</label>
						</div>
						<div class="col-sm-3 text-left">
							<s:radio name="invoiceCreditMasterBean.invoiceType" list="priceTypeList"
						...
						<div class="col-sm-3 text-left">
							<s:select id="invoiceStatus" name="invoiceStatus" cssClass="form-control"
		                      list="statusCombo" listKey="code" listValue="desc" value="%{invoiceCreditMasterBean.invoiceStatus}" 
		                      onchange="lp_setStatus();" />
		                    <s:hidden name="invoiceCreditMasterBean.invoiceStatus" id="invoiceCreditMasterBean.invoiceStatus"/>
						</div>
					</div>
					<%--End Row 3 --%>
				  </div>
				...
				<div class="panel panel-primary" style="border: none;box-shadow: none;">
					...
					<div class="panel-body paddingForm" id="invoiceCredit-list">
						<%@include file="invoiceCredit-list.jsp" %>
					</div>
				</div>
				<div class="panel" style="border: none;box-shadow: none;">
				  <div class="panel-body paddingForm">
					...
					<div class="col-sm-12">
						...
						<div class="col-sm-3 text-left">
							<s:textfield id="invoicePrice" name="invoiceCreditMasterBean.invoicePrice" cssClass="form-control moneyOnly" onblur="lp_calInvoiceVat();" placeholder="รวมจำนวนเงิน" />
						</div>
					</div>
					<%--End Row 1 --%>
					<%--Begin Row 2 --%>
					<div class="col-sm-12">
						...
						<div class="col-sm-3 text-left">
							<s:textfield id="invoicediscount" name="invoiceCreditMasterBean.invoicediscount" cssClass="form-control moneyOnly" onblur="lp_calInvoiceVat();" placeholder="หักส่วนลด" />
						</div>
					</div>
					<%--End Row 2 --%>
					<%--Begin Row 3 --%>
					<div class="col-sm-12">
						...
						<div class="col-sm-3 text-left">
							<s:textfield id="invoiceVat" name="invoiceCreditMasterBean.invoiceVat" cssClass="form-control moneyOnly" onblur="lp_InvoiceTotal();" placeholder="VAT" />
						</div>
					</div>
					<%--End Row 3 --%>
					<%--Begin Row 4 --%>
					<div class="col-sm-12">
						...
						<div class="col-sm-3 text-left">
							<s:textfield id="invoiceDeposit" name="invoiceCreditMasterBean.invoiceDeposit" cssClass="form-control moneyOnly" onblur="lp_InvoiceTotal();" placeholder="หักมัดจำ" />
						</div>
					</div>
					<%--End Row 4 --%>
					<%--Begin Row 5 --%>
					<div class="col-sm-12">
						...
						<div class="col-sm-3 text-left">
							<s:textfield id="invoiceTotal" name="invoiceCreditMasterBean.invoiceTotal" cssClass="form-control moneyOnly" placeholder="รวมทั้งสิ้น" />
						</div>
					</div>
					<%--End Row 5 --%>
					<%--Begin Row 6 --%>
					...
					<%--Begin Row 7 --%>
					<div class="col-sm-12">
						<s:textarea id="remark" name="invoiceCreditMasterBean.remark" cols="60" rows="4" cssClass="form-control"></s:textarea>
					</div>
					<%--End Row 7 --%>
					<%--Begin Row 8 --%>
					<div class="col-sm-12">
						<div class="col-sm-12 text-right">
						    ...
						    <s:else>
						    	<s:if test='!"C".equals(invoiceCreditMasterBean.invoiceStatus) && !"S".equals(invoiceCreditMasterBean.invoiceStatus)'>
