...
<table class="table table-hover table-bordered table-criteria">
    <thead>
      <tr>
        <th width="5%" >ลำดับ</th>
		<th width="17%">เลขที่ใบเสร็จ</th>
		<%--<th width="15%">เลขที่ใบวางบิล</th> --%>
		<th width="14%">ประเภทการขาย</th>
		<th width="23%">ลูกค้า</th>
		<th width="12%">วันที่ขาย</th>
    ...
    <tbody>
    	<s:if test="!paginatedList.list.isEmpty">
    		<s:iterator status="stat" value="paginatedList.list" var="item">
	    		<tr onclick="lp_gotoDetail('<s:property value="#item.invoiceCode"/>');">
					...
					<%--
					<s:if test='"".equals(#item.invoiceCredit)'>
						<td align="center">-</td>
					</s:if>
					<s:else>
						<td><s:property value="#item.invoiceCredit"/></td>
					</s:else>
					--%>
					<td><s:property value="#item.invoiceTypeDesc"/></td>
					<s:if test='"".equals(#item.cusFullName)'>
						<td align="center">-</td>
					</s:if>
					<s:else>
					...
					<td><s:property value="#item.invoiceStatusDesc"/></td>
				</tr>
			</s:iterator>
    	</s:if>
    	<s:else>
